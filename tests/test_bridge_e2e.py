...
def test_worker_module_imports_without_driver_only_pipelines() -> None:
    script = """
...
class BlockDriverOnlyImports(importlib.abc.MetaPathFinder):
    def find_spec(self, fullname: str, path: object = None, target: object = None) -> None:
        if (
            fullname in DRIVER_ONLY_MODULES
            or fullname == "vllm"
            ...
            raise ModuleNotFoundError(f"blocked driver-only import: {fullname}")
        return None

sys.meta_path.insert(0, BlockDriverOnlyImports())
from rdan_grpo.workers import ResponseActorWorker, ResponseVLLMInferWorker

assert ResponseActorWorker.__name__ == "ResponseActorWorker"
assert ResponseVLLMInferWorker.__name__ == "ResponseVLLMInferWorker"
assert not any(name in sys.modules for name in DRIVER_ONLY_MODULES)
...
def test_pinned_roll_adapter_installs_at_real_pipeline_boundary() -> None:
    ...
    adapter = __import__("rdan_grpo.bridge", fromlist=["install_roll_adapter"]).install_roll_adapter(certificate)
