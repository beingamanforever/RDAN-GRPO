...
from rdan_grpo.bridge import PreflightCertificate, make_roll_compute_advantage
from rdan_grpo.config import UPDATES_PER_STEP
from rdan_grpo.scalar import QUALITY_METHODS, ScalarMethod
...
def run_response_train_step(
    *,
    ...
    certificate: PreflightCertificate | Mapping[str, Any] | None,
    ...
    returns, world_size = _validate_topology(pipeline_config, actor_train, actor_infer)
    prompt_count = _validate_rewarded_batch(rewarded_batch, returns)
    state_before = _training_state(observe_training_state(), "before", world_size)
    ...
    metrics, optimizer_delta, scheduler_delta = _execute_actor_update(
        actor_train,
        rewarded_batch,
        full_response_mask,
        state_before,
        observe_training_state,
        world_size,
    )
    peak_fraction = _validate_post_transaction_memory(observe_post_transaction_memory(), world_size)
...
def _execute_actor_update(
    ...
    state_after = _training_state(observe_training_state(), "after", world_size)
    optimizer_delta, scheduler_delta = _validate_state_delta(state_before, state_after)
    return metrics, optimizer_delta, scheduler_delta
...
def _validate_topology(config: Any, actor_train: Any, actor_infer: Any) -> tuple[int, int]:
    expected = (
    ...
    for worker_config, cluster, strategy_name, name in expected:
        ...
        devices = _worker_devices(worker_config, name)
        _validate_cluster(cluster, name, len(devices))
        device_mappings.append(devices)
    if device_mappings[0] != device_mappings[1]:
        raise RuntimeError("response training requires actor_train and actor_infer colocated on the same devices")
    if (
    ...
    return returns, len(device_mappings[0])


def _worker_devices(worker_config: Any, name: str) -> list[int]:
    devices = list(getattr(worker_config, "device_mapping", None) or ())
    if not devices or any(isinstance(device, bool) or not isinstance(device, int) or device < 0 for device in devices):
        raise RuntimeError(f"response training requires {name} device_mapping to list GPU indices")
    if len(set(devices)) != len(devices):
        raise RuntimeError(f"response training requires {name} device_mapping without repeated devices")
    if getattr(worker_config, "num_gpus_per_worker", None) != 1:
        raise RuntimeError(f"response training requires {name} num_gpus_per_worker=1")
    if getattr(worker_config, "world_size", None) != len(devices):
        raise RuntimeError(f"response training requires {name} world_size equal to its device count")
    return devices


def _validate_cluster(cluster: Any, name: str, world_size: int) -> None:
    if getattr(cluster, "dp_size", None) != world_size:
        raise RuntimeError(f"response training requires live {name} dp_size {world_size}")
    ranks = getattr(cluster, "worker_rank_info", None)
    if not isinstance(ranks, Sequence) or len(ranks) != world_size:
        raise RuntimeError(f"response training requires {world_size} live {name} workers")
    for rank in ranks:
        if (
            getattr(rank, "dp_size", None) != world_size
            or getattr(rank, "tp_size", None) != 1
            or getattr(rank, "pp_size", None) != 1
            or getattr(rank, "cp_size", 1) != 1
        ):
            raise RuntimeError(f"response training requires live {name} data parallelism with TP1 PP1 CP1")
...
def _training_state(values: Sequence[Mapping[str, Any]], phase: str, world_size: int) -> dict[int, Mapping[str, Any]]:
    if not isinstance(values, Sequence) or isinstance(values, (str, bytes)) or len(values) != world_size:
        raise RuntimeError(f"{phase} training state must contain all {world_size} actor ranks")
...
def _validate_state_delta(
    ...
    ranks = sorted(before)
    optimizer_deltas = {after[rank]["optimizer_step"] - before[rank]["optimizer_step"] for rank in ranks}
    scheduler_deltas = {after[rank]["scheduler_step"] - before[rank]["scheduler_step"] for rank in ranks}
    if len(optimizer_deltas) != 1 or len(scheduler_deltas) != 1:
        raise RuntimeError("actor ranks disagree on the optimizer or scheduler step delta")
    optimizer_delta = optimizer_deltas.pop()
    scheduler_delta = scheduler_deltas.pop()
    if optimizer_delta != UPDATES_PER_STEP or scheduler_delta != UPDATES_PER_STEP:
        raise RuntimeError(f"optimizer and scheduler state did not follow the {UPDATES_PER_STEP}-update cadence")
    return optimizer_delta, scheduler_delta


def _validate_post_transaction_memory(values: Sequence[Mapping[str, Any]], world_size: int) -> float:
    if not isinstance(values, Sequence) or isinstance(values, (str, bytes)) or len(values) != world_size:
        raise RuntimeError(f"post-transaction memory must contain all {world_size} GPUs")
