...
PROGRAM_CONFIG = ROOT / "configs/program/qwen_first.json"
    ...
    def _evaluate_hir_hard(
        ...
        try:
            if source in {"type1", "type2"}:
                ...
                if not result.valid:
                    error = RuntimeError(f"safe_rule:{result.error or 'invalid'}")
...
def _load_program_seed() -> int:
    seed = json.loads(PROGRAM_CONFIG.read_text(encoding="utf-8")).get("seeds", {}).get("tuning")
    if not isinstance(seed, int) or isinstance(seed, bool):
        raise RuntimeError("program tuning seed is absent or invalid")
    return seed
