...
from rdan_grpo.scalar import QUALITY_METHODS, ScalarMethod, ScalarOutput, build_scalar_output
...
def inject_roll_advantages(
    ...
    The certificate is a launch gate only: it contributes nothing to the advantage math
    below, which depends solely on ``output`` and ``response_mask``. Passing ``None`` skips
    the gate checks entirely and trains ungated, matching the upstream RTT/PAPO launch story.
    """
...
def make_roll_compute_advantage(
    ...
    *,
    ...
    ``certificate=None`` skips the readiness and method-binding gate checks and trains
    ungated; the certificate never contributes to the advantage computation itself.
    """
