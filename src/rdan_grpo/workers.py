"""Production RTT workers for response-level FSDP2 training and vLLM rollout."""
...
class ResponseActorWorker(ActorWorker):
    """Train through RTT RLVR while observing exact optimizer and checkpoint state."""
    ...
    def start_model_update(self, model_update_name: str) -> DataProto:
        """Run a named FSDP2 model update against the colocated vLLM inference engine."""
        ...
        return super().start_model_update(model_update_name)
...
def _rank(worker: Any) -> int:
    ...
    if isinstance(rank, bool) or not isinstance(rank, int) or rank not in {0, 1}:
        raise RuntimeError("response worker requires actor or inference DP rank 0 or 1")
    return rank
