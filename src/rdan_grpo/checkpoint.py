...
_OPTIONAL_IDENTITY_KEYS = {"certificate"}
_MANIFEST_KEYS = {
    "schema_version",
    "status",
    "completed_step",
...
class CheckpointIdentity:
    ...
    ``certificate`` is the launch gate identity and is optional: an ungated run passes
    ``None`` and the serialized manifest omits the certificate block entirely, so a gated
    and an ungated checkpoint remain schema-distinguishable and neither loads as the other.
    """
    ...
    certificate: ArtifactIdentity | None
...
def load_checkpoint(path: str | Path, *, identity: CheckpointIdentity) -> dict[str, Any]:
    ...
    manifest_has_certificate = isinstance(manifest, dict) and "certificate" in manifest
    expected_manifest_keys = _MANIFEST_KEYS | (_OPTIONAL_IDENTITY_KEYS if manifest_has_certificate else set())
    ...
    if manifest_has_certificate != ("certificate" in expected_identity):
        raise CheckpointError("checkpoint certificate gating does not match this run identity")
    identity_keys = _IDENTITY_KEYS | (_OPTIONAL_IDENTITY_KEYS if manifest_has_certificate else set())
    if {key: manifest[key] for key in identity_keys} != expected_identity:
